function [hddm, Num_excluded] = hddm_input(sub, choice_data, choice_text, black_data, black_text, rate)
    ...
    rtlow = 0.2;
    rthigh = mean(rt)+3*std(rt);
    
    response = acc;
    subj_idx = ones(length(choice_data),1) * (sub-1);
    
    hddm_raw = table(subj_idx, subj, rt, response, dist, diff, condition, block, trial);
    hddm = hddm_raw((find(rt>rtlow & rt < rthigh)),:);
    Num_excluded = height(hddm_raw)-height(hddm);
